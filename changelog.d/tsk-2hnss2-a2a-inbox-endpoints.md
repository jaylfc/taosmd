...
### Fixed
* `a2a_inbox_unhandled` now applies the `limit` after filtering out acknowledged messages, so acked messages no longer consume the caller's budget and cause false-empty results.
