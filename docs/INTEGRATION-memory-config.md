...
| `generator_profile` | consumer (answer-gen) | `generator_profile` | `balanced` | Selects the answer generator by workload per hardware tier. `balanced` / `factual-recall` (granite4.1:8b at 12/8 GB for single-fact QA; loses conversational/long-context work). |
