...
| E-014 | MemX R1 low-confidence reject gate (abstain when no lexical hit AND max dense cosine < tau=0.50): what is its recall cost on LoCoMo? Ships as an opt-in low-confidence mode only if R@K drop <= 0.02; otherwise a negative result (too aggressive for answerable LoCoMo, default-off) | [6](#6-ongoing-work-pre-registered) | pre-registered | feat/memx-levers 96b7725, taosmd/retrieval.py apply_reject_gate |
...
Status: pre-registered, ablation running on the bench host (judge-free, CPU).
