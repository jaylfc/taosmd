...
| F-011 | Claims gate (prefer_verified) eliminates served-hallucination (0.040 to 0.000) at no accuracy cost (judge +0.020 within noise, R@K -0.005) on 200 LoCoMo QAs; strict trades too much (judge/R@K -0.065), stays opt-in. Tri-judge confirm (lenient gemma4:e2b, 2026-06-18) corroborates no-harm but did NOT reproduce the benefit (that config had 0.000 served-hall at baseline, nothing to remove); benefit is config-dependent. Default flip pending Jay sign-off; reproduced at n=250 by E-018 (2026-06-20) under the strict llama judge (served-hall 0.040 to 0.000, judge -0.016, R@K -0.008, all within bounds), so it now rests on two strict-llama runs, but the tri-judge clause is still open | [6](#6-ongoing-work-pre-registered) | resolved (flip pending Jay; reinforced at n=250, tri-judge open) | bench host benchmarks/results/e009.json + e009_gemma_20260618_102802.json + e018_e009_20260619_233528.json |
...
| E-016 | External temporal-memory benchmark: BiTempQA did not verify; TEMPO ruled out (document-retrieval, NDCG-scored, NOT memory-shaped per the kill criterion); RETARGETED to MemoryAgentBench (ICLR 2026, arXiv:2507.05257, EventQA = temporal event chains, multi-turn inject-once-query-many, open-source) which passes the feasibility gate. Does the E-012 self-verify temporal lift transfer? RAN 2026-06-20 (off 19.0% / on 16.0%) but the result is INVALID: ~half the generations degenerated to 1-2 token stubs on the long contexts, both arms at/below the 16.7% six-way-MC chance floor. Harness-blocked; transfer question open; N-013 stays the self-verify-transfer evidence | [6](#6-ongoing-work-pre-registered) | ran but INVALID (generation artifact); harness fix pending | feat/eventqa-runner 9809430 |
...
| E-018 | E-009 prefer_verified larger-n confirm on the served-hallucination config: at n=250 under the strict llama3.1:8b judge, prefer_verified eliminates served-hall (0.040 to 0.000) at judge -0.016 / R@K -0.008 (both within 0.02); strict over-trades (judge -0.024 / R@K -0.060). Reproduces F-011 at larger n; only the llama judge ran, so the tri-judge clause is open and the flip stays Jay's call | [6](#6-ongoing-work-pre-registered) | resolved (larger-n strict-llama confirm; tri-judge clause open) | bench host e018_e009_20260619_233528.json |
