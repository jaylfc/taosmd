...
| N-014 | BEAM harness empty-answer confound: the runner's 60s LLM timeout returns empty answers on slow (CPU) generation, and mem0's nugget judge scores an empty answer as PASS on negative/abstention rubrics, so blanks inflate the score. Caught on the VPS 30B-A3B CPU run: first pass 55% with 10/20 empties, honest re-run (240s, 0 empties) 20% (n=20). Code-confirmed (beam_runner.py timeout=60, judge prompt line 431). GPU 9B numbers (errors=0, fast gen) are probably unaffected but are pre-registered for hardened re-confirm (E-013) | [4](#4-negative-results) | confirmed (methodology) | task report 2026-06-18 VPS 30B run; beam_runner.py feat/beam-runner |
...
Status: pre-registered, queued for the next GPU window after the E-009 claims-gate confirm releases the 3060.
