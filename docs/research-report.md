...
| E-015 | MemX reject-gate completion: tau-sweep (0.50->0.85) + FTS-source test to map firing and recall-cost curve, completing N-015. Ships as a useful opt-in only if some tau cuts low-confidence served answers at R@K drop <= 0.02 | [6](#6-ongoing-work-pre-registered) | pre-registered | feat/memx-levers |
...
Status: E-015 to E-018 pre-registered. Running paced: E-015 first (judge-free, CPU), then E-017 and E-018 on the GPU sequentially, and E-016 after a dataset feasibility check plus a subagent harness build.
