...
Status: pre-registered, queued. Judge-free and CPU-bound (ONNX embedding + retrieval, no LLM), so it can run concurrently with GPU LLM work without contention, or on the VPS. Runs after E-009 records.
...
Status: BASELINE recorded (F-012). Improvement phase in progress (Jay: "use all our new knowledge to get the scores up"). The baseline starved the generator of evidence (assembler 2000 tokens, 5 vector chunks, 3 FTS hits), which directly explains the multi-session/temporal collapse. The harness depth knobs are now tunable (TAOSMD_ASSEMBLE_TOKENS, RETRIEVE_LIMIT, FTS_LIMIT, CONTEXT_CHARS; commit a23ea26) for a richer-evidence sweep, plus arctic-embed-s and query-expansion levers, plus a lenient/tri-judge pass for a comparable number and the full-distractor set. Each improvement is measured against this 47.2% baseline and recorded. GPU runs are coordinated with @taOS via the shared-hardware lease protocol.
