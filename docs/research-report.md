...
| E-016 | External temporal-memory benchmark: BiTempQA did not verify; TEMPO ruled out (document-retrieval, NDCG-scored, NOT memory-shaped per the kill criterion); RETARGETED to MemoryAgentBench (ICLR 2026, arXiv:2507.05257, EventQA = temporal event chains, multi-turn inject-once-query-many, open-source) which passes the feasibility gate. Does the E-012 self-verify temporal lift transfer? Harness build via subagent | [6](#6-ongoing-work-pre-registered) | pre-registered (retargeted, harness in progress) | github HUST-AI-HYZ/MemoryAgentBench |
