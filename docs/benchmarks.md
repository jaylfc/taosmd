...
**CORRECTION (2026-06-23, N-017).** The F-012 47.2% and F-013 74.6% figures in the table below were inflated by a verdict-parser bug: the judge parser counted INCORRECT replies as passes (the string "INCORRECT" contains "CORRECT"), so the Judge score tracked non-abstention rather than correctness. Fixed in PR #176. The corrected full-500 end-to-end Judge on the shipped qwen3.5:9b config is **42.8%** on the strict Qwen judge and **51.2%** on a cross-family llama judge. A gemma4:12b generator upgrade lifts it to **53.8%** on the strict Qwen judge and **61.4%** on the cross-family llama judge (full-500), bringing the taOSmd combo to PARITY with MemOS-lossless on a like-for-like head-to-head: at full-500 it is a statistical tie (combo 53.8 / 61.4 vs MemOS 54.0 / 61.2 on the two judges). Generator note: this gemma4:12b win is task-dependent. It wins on LongMemEval but UNDERperforms the shipped qwen3.5:9b on LoCoMo (0.63 vs 0.68, N-007) and BEAM (46% vs 49%), so the shipped default generator stays qwen3.5:9b (the multi-session and long-context leader); gemma4:12b is the better pick only for single-fact retrieval-QA workloads like LongMemEval. The original ablation table is retained below for the record; every Judge figure in it is inflated by this bug. Full provenance in the [research report](research-report.md) (N-017).
...
- **gemma4:12b is not a generator upgrade.** On the identical subset-200 dense-baseline config the week-after-release gemma4:12b scored 0.630 under the lenient gemma4:e2b judge and 0.580 under strict qwen3:4b-instruct-2507, against qwen3.5:9b's 0.680 on the same subset. The lenient judge shares the generator's model family here, which would inflate rather than hide a win, so the deficit is credible. The first attempt at this A/B also produced a methods lesson: a missing Ollama model was stored as 200 zero-scoring generation-error predictions and impersonated a catastrophically bad generator until the runner was fixed to abort loudly (results files 20260611_*_gemma4_12b_baseline_s200 on the bench host).
...
**Wider-retrieval coverage lever (2026-07-10, N-020 in the research report).** On the shipped combo config (subset-200, external qwen3:4b judge, base 0.535), widening retrieval to a k=100 candidate pool with top-20 final raised judge-free retrieval coverage (MultiHop evidence-hit coverage 0.434 to 0.576) but left end-to-end accuracy overall-neutral at 0.545 (+0.010, within noise). Per-category it is a trade, not a uniform lift: MultiHop rose 0.209 to 0.302 (the coverage converting to answers) but Temporal fell 0.619 to 0.571, cancelling it. Coverage is necessary but not sufficient; wider pool is a per-workload MultiHop-only option, not a default change. Not a truncation artifact (answer-step num_ctx is 16384). Full detail in the research report (E-027, N-020, F-018).

**Combo-lever sweep and the category-tension (2026-07-11, E-028 in the research report).** A five-arm sweep on the same base (subset-200, external qwen3:4b judge, base 0.535) generalises the trade above. Context-based MultiHop levers (timeline reformatting, wider retrieval) reliably lift MultiHop by +0.09 to +0.16 but cost Temporal, so they net within noise overall (timeline 0.515, timeline+wide 0.555). RRF fusion regresses to 0.480 (mem0_additive beats RRF at generation). The one arm that breaks the tension is `--force-empty-think` (a prompt-framing lever: an empty `<think></think>` block plus `think:false`, genuinely think-off), which lifts MultiHop +0.140 (0.209 to 0.349) without hurting Temporal (+0.016) for overall 0.570 at 3.1s/QA. It is recorded as a promising-but-unconfirmed MultiHop candidate (overall +0.035 sits at the subset-200 noise boundary, no fixed seed); NOT shipped, defaults unchanged, with a seed-varied confirm pre-registered. Separately, capping the E-026 thinking config's output (`--num-predict 512`) breaks it (99% empty answers), so the thinking lever is dead. Full detail in the research report (E-028, N-021, N-022, N-023).
...
> - **Best factual recall (single-fact QA)** → the shipped `factual-recall` generator profile: `granite4.1:8b` at the 12 and 8 GB tiers (54.8 Qwen / 60.4 llama on the corrected LongMemEval-S full-500, F-016; 4.9 GB q4, Apache-2.0), `llama3.1:8b` at the 4 GB tier (E-023/F-015). granite is a statistical tie with the prior 12 GB pick `gemma4:12b` (53.8 / 61.4, N-017); the VRAM headroom and license break the tie. LoCoMo caveat: granite fails the balanced-default kill criterion (0.475 vs 0.515 external judge, E-024), so this stays a single-fact-QA profile. The earlier `llama3.1:8b` Single-hop pick (0.65) stands as the LoCoMo-era measurement that seeded this profile.
> - **Best temporal reasoning** → `mistral-small3.2` (0.71 Temporal, but 2.8× slower than qwen)
> - **Production default**: still `qwen3.5:9b` (the `balanced` profile) because single-fact QA is one workload among several; broad workloads should optimise for Overall. Both `gemma4:12b` (LoCoMo 0.63 vs 0.68, BEAM 46 vs 49) and `granite4.1:8b` (LoCoMo 0.475 vs 0.515, E-024) lose on conversational/long-context work, so they stay per-task picks, not the default.
...
**July 2026 factual-recall update (F-016/E-024).** granite4.1:8b on the shipped combo config scores **54.8** on the strict Qwen judge and **60.4** on the cross-family llama judge (LongMemEval-S full-500), a statistical tie with the gemma4:12b combo (53.8 / 61.4) and MemOS-lossless (54.0 / 61.2) at 4.9 GB q4 under Apache-2.0. It failed the pre-registered LoCoMo kill criterion (0.475 vs qwen3.5:9b 0.515, external qwen3:4b judge; wins SingleHop, loses Temporal and OpenDomain), so it ships only as the `factual-recall` profile pick at the 12 and 8 GB tiers, and the `balanced` default stays qwen3.5:9b. Provenance: `benchmarks/results/20260705-screening/` (screen summary, chain status, rescore logs), `benchmarks/results/20260706-locomo-granite/`, research report F-016/E-024.
