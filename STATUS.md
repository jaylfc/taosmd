...
4. **E-011 BLOCKED: the 97.0% headline does NOT reproduce with the obvious config.** arm 1 (MiniLM + qwen2.5:3b gen+judge + the substituted oracle + default retrieval) scored **22.6%**, not ~97.0% (`benchmarks/results/e011_minilm.log`). Per-category proves it is the wrong recipe, not noise: temporal 3.8% (headline 94.0%), multi-session 7.5% (headline 98.5%), single-session-assistant 75%. Retrieval is fast/fine (ingest 0.3-0.6s); the failure is generator/recipe. ROOT-CAUSE CANDIDATES (un-pinned): (a) the runner's CODE-DEFAULT qwen2.5:3b is almost certainly NOT the headline generator (a 3B can't do 94% temporal) -> the 97.0% used a bigger generator via undocumented env override; (b) 97.0% is the "hybrid + query expansion (default)" path (benchmarks.md fusion table) which the plain runner does not enable; (c) the ORIGINAL oracle dataset is gone (dead symlink -> ~/tinyagentos, now missing; I substituted ~/taos-browser-worker's copy which may differ). **DO NOT launch arm 2 until the MiniLM arm reproduces ~97.0%.** REAL GAP: the flagship 97.0% is not currently reproducible from the repo as-is; its exact recipe (generator, retrieval flags, dataset file) is undocumented. Surfaced to Jay. Next: get the actual 97.0% recipe (ask Jay / dig old result files), reproduce, THEN E-011. Arm-2 arctic command staged once baseline is valid (arctic onnx at /home/jay/.cache/huggingface/hub/models--Snowflake--snowflake-arctic-embed-s/snapshots/e596f507467533e48a2e17c007f0e1dacc837b33).
