...
- **E-008 RESOLVED to N-011: surprisal is the WORST retention signal (below random at every budget); length wins.** The v2 surprisal pillar is conclusively dead across retrieval (N-009/N-010) and consolidation (N-011), so the v2 spine PIVOTS to the provenance/claims layer (anchored by F-009, 18.8 percent extraction-hallucination rate, unmatched), per the pre-registered criterion and Jay's plan (report rev 1.14). Surprisal P1 plan shelved. **v2 claims-layer (Provable Memory): brainstormed + design APPROVED + implementation plan WRITTEN (both private: ~/tinyagentos-private/specs|plans/2026-06-13-v2-claims-layer-*).** MVP = verify extracted claims vs their archive spans (cross-family local LLM, async/idle verify-pass, demote-not-delete gate, additive default-off), turning F-009 into a live gate; validation = pre-register E-009 before flipping the flag. NEXT: execute the 9-task plan (taosmd/claims/ package). Side-finding: length is a strong free retention heuristic, flagged for its own pre-registration.
