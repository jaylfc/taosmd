...
4. **E-011 arctic on LongMemEval-S** (pre-registered rev 1.17): does MiniLM->arctic move the 97.0% headline? **GPU now FREE (@taOS done).** SETUP REQUIRED before a meaningful launch (do NOT rush-launch): (a) the 97.0% generator+judge config is NOT documented in benchmarks.md and `longmemeval_runner.py` uses ONE model (`REMOTE_LLM_MODEL`, default qwen2.5:3b, NOT pulled on Fedora) for BOTH generation and judging = same-model inflation (F-008). (b) To claim "moves the headline" the MiniLM arm must reproduce ~97.0% under the original config -> first find that config (git history / old longmemeval result files on the bench archive) OR pick a documented generator and re-baseline. (c) Best practice: add a `TAOSMD_JUDGE_MODEL` env split to the runner so the judge is EXTERNAL to the generator, then run both arms (TAOSMD_ONNX_PATH=minilm-onnx vs arctic-embed-s dir; arctic prefix/CLS auto-applies by path name) full-500, checkpointed. Same-model judge is acceptable for the relative MiniLM-vs-arctic DELTA but not for an honest absolute headline claim. ACK'd to @taOS that I'll run it deliberately.
