...
- **E-007 arctic-embed (low-tier dense embedder): CONFIRMED on subset-200 (F-010), full-1540 running on Fedora.** Both stages passed: R@K arctic-s 0.8333 vs MiniLM 0.6768 (+0.157), judged arctic-s 0.7150 vs 0.6750 (+0.040, qwen3.5:9b gen, gemma4:e2b), same 384 dim and latency (report rev 1.13, section 3.7). Arctic ONNX support (query-only prefix + CLS pooling) merged PR #160, default-off. The full-1540 judged confirm (both arms, checkpointed via --ckpt) is running on the GPU before the user-facing default is switched; then a small Pi-CPU speed sample (PSU lesson, memory feedback_pi_speed_only) and a default-flip proposal. First positive finding after N-008/N-009/N-010.
- **E-008 RESOLVED to N-011: surprisal is the WORST retention signal (below random at every budget); length wins.** The v2 surprisal pillar is now conclusively dead across retrieval (N-009/N-010) and consolidation (N-011), so the v2 spine PIVOTS to the provenance/claims layer (anchored by F-009, the 18.8 percent extraction-hallucination rate, unmatched by competitors), per the pre-registered criterion and Jay's plan (report rev 1.14). The surprisal P1 plan is shelved. NEXT: lock the claims-layer MVP shape (brainstorm vs draft, Jay's call), then build. Side-finding: length is a strong free retention heuristic, flagged for its own pre-registration.
- **PR #161 (recipe-aware store construction, #34 Gap 1) awaiting Jay sign-off, NOT self-merged.** Closes the gap where the lateint-9b recipe was recommended for the 8 GB tier but the live store was always built dense: storage mode (late_interaction/colbert_model) now lives in config.vector_memory, seeded from recommend() at setup, honored in _ensure_stores, with a store_meta marker + StoreModeMismatch fail-loud guard. 857 tests green, dense default unchanged. Same config seam will carry the arctic embedder (Gap 2) once E-007 full-1540 confirms.
