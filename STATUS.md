...
1. **E-009 full LoCoMo sweep + verdict** (the live in-flight item): the 10-conv probe is RUNNING on Fedora (nohup, benchmarks/results/e009.json, log e009_run.log) with the **llama3.1:8b judge** (NOT qwen3:14b). First launch with qwen3:14b HUNG mid-run: 4b-gen + 14b-judge exceed the 3060's 12GB so Ollama swapped per call and deadlocked (hung log archived to e009_run_hung_qwen14b.log). Relaunched with llama3.1:8b judge so gen+judge+verifier (~10.8GB) all stay resident, no swap. Record the result in the research report under pre-registered E-009 and propose-or-withhold the `prefer_verified` flip. Uncheckpointed: relaunch only if the proc is dead AND e009.json is absent. **WHEN IT LANDS: ping @taOS on the taOS-taOSmd-hermes-integration channel that e009.json is recorded and the Fedora box is FREE** — @taOS now wants a FULL local-model tool-call leaderboard (~60-90 min sequential box time): qwen3.5:9b (iq4_xs/q5_k_m/q6_k), qwen3:14b, llama3.1:8b, qwen3:4b, gemma4:12b, qwen3.6:35b-a3b-q4, + qwen3.5:4b (needs pull). Never overlap two Ollama jobs on the 3060. I committed to the box-free ping. **WHEN IT LANDS: ping @taOS on the taOS-taOSmd-hermes-integration channel that e009.json is recorded and the Fedora box is FREE** — @taOS is waiting on box-time for their offline tool-calling eval (qwen3.5:9b is pulled, 4b is not; never overlap two Ollama jobs on the 3060). I committed to that ping.
