...
4. **E-011 arctic on LongMemEval-S RUNNING (arm 1).** Config pinned from git history: the 97.0% used `qwen2.5:3b` for BOTH generate and judge (same-model, on Fedora) on the ORACLE set (`longmemeval_oracle.json`). **arm 1 (MiniLM baseline) launched on Fedora** (`benchmarks/results/e011_minilm.log`, TAOSMD_OLLAMA_MODEL=qwen2.5:3b TAOSMD_ONNX_PATH=models/minilm-onnx --llm --limit 500); should reproduce ~97.0% and validate the setup. **WHEN arm 1 finishes, launch arm 2 (arctic)**: `cd /home/jay/taosmd && TAOSMD_OLLAMA_MODEL=qwen2.5:3b TAOSMD_ONNX_PATH=/home/jay/.cache/huggingface/hub/models--Snowflake--snowflake-arctic-embed-s/snapshots/e596f507467533e48a2e17c007f0e1dacc837b33 nohup timeout 9000 .venv/bin/python benchmarks/longmemeval_runner.py --llm --limit 500 > benchmarks/results/e011_arctic.log 2>&1 &` (path contains "arctic" so prefix+CLS auto-apply). Then read both overall %; delta = E-011 answer. GOTCHAS FIXED: qwen2.5:3b pulled; oracle was a broken symlink (repointed to /home/jay/taos-browser-worker/benchmarks/data/). HONEST CAVEAT for the report: the 97.0% (hence E-011) uses same-model qwen2.5:3b judge (inflates per F-008) on the easy oracle set, so arctic's retrieval edge may show little there; if so that is itself the finding (arctic helps hard retrieval, oracle is easy).
