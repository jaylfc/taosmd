...
# ---------------------------------------------------------------------------
# DEFECT 1 guard: every public a2a_* coroutine must appear in service.__all__
# ---------------------------------------------------------------------------

class TestA2aExports:
    """RED-FIRST gate: would have caught a2a_sender_census being dropped from
    service.__all__ on the PR #336 branch."""

    def test_all_public_a2a_coroutines_in_all(self):
        a2a_coroutines = {
            name
            for name, obj in inspect.getmembers(service)
            if inspect.iscoroutinefunction(obj) and name.startswith("a2a_")
        }
        missing = a2a_coroutines - set(service.__all__)
        assert not missing, (
            f"Public a2a_* coroutine(s) missing from service.__all__: {sorted(missing)}"
        )

    def test_all_entries_in_all_resolve_to_callables(self):
        for name in service.__all__:
            if name.startswith("a2a_"):
                assert hasattr(service, name), f"{name} listed in __all__ but not defined in service"
...
def test_http_mentions_reader_mismatch_returns_403(authed_server):
    ...
    bob_token = _make_token("bob", iss=REGISTRY_ISS)
    agent_a_token = _make_token("agentA", iss=REGISTRY_ISS)
    # Send with agentA's own token so the message is stored (DEFECT 3 fix).
    _post(f"{authed_server}/a2a/send",
          {"from": "agentA", "body": "secret for @bob", "thread": "cross"}, token=agent_a_token)

    # Positive control: bob's own token sees the mention.
    s_bob, body_bob = _get(f"{authed_server}/a2a/mentions", token=bob_token)
    assert s_bob == 200, body_bob
    assert any(m["body"] == "secret for @bob" for m in body_bob["messages"])

    # Arm: carol's token with ?reader=bob must fail closed.
    s, body = _get(f"{authed_server}/a2a/mentions?reader=bob", token=carol_token)
    assert s == 403, body
    assert "secret for @bob" not in json.dumps(body)


def test_http_mentions_reader_normalised_handle_accepted(authed_server):
    """?reader=@bob with bob's token returns 200; proves handle is normalised before auth."""
    bob_token = _make_token("bob", iss=REGISTRY_ISS)
    agent_a_token = _make_token("agentA", iss=REGISTRY_ISS)
    _post(f"{authed_server}/a2a/send",
          {"from": "agentA", "body": "note for @bob", "thread": "cross"}, token=agent_a_token)

    # All four variants normalise to "bob"; the @-prefixed variant is the DEFECT 2 arm.
    for qp_reader in ("bob", "@bob", "Bob", "@Bob"):
        s, body = _get(f"{authed_server}/a2a/mentions?reader={qp_reader}", token=bob_token)
        assert s == 200, f"qp_reader={qp_reader!r}: {body}"
        assert any(m["body"] == "note for @bob" for m in body["messages"])

    # Disagreement control: wrong token still 403 even with matching handle.
    carol_token = _make_token("carol", iss=REGISTRY_ISS)
    s, body = _get(f"{authed_server}/a2a/mentions?reader=bob", token=carol_token)
    assert s == 403, body
