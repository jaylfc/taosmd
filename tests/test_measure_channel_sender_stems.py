"""Tests for channel-sender stem measurement."""
...
from scripts.measure_channel_sender_stems import (
...
class TestMeasure:
    ...
    def test_two_canonicals_share_one_stem_without_mint(self):
        ...
        result = measure(pairs)
        # Both are canonicals; each has a twin of the other form
        assert len(result["multi_spell_stems_without_mint"]) == 1
        assert "taosmd-20260609-153000" in result["multi_spell_stems_without_mint"]
...
class TestAsyncMain:
    def test_data_dir_required(self, tmp_path):
        """--data-dir is required; missing argument exits non-zero."""
        result = subprocess.run(
            [sys.executable, "scripts/measure_channel_sender_stems.py"],
            capture_output=True,
            text=True,
            timeout=60,
        )
    ...
    def test_data_dir_with_no_rows_exits_nonzero(self, tmp_path):
        ...
        empty_dir.mkdir()
        result = subprocess.run(
            [sys.executable, "scripts/measure_channel_sender_stems.py", "--data-dir", str(empty_dir)],
            capture_output=True,
            text=True,
            timeout=60,
        )
    ...
    def test_insufficient_data_with_few_principals(self, tmp_path, monkeypatch):
        """Few principals (< threshold) produce INSUFFICIENT DATA, not SAFE."""
        data_dir = tmp_path / "taosmd-test"
        data_dir.mkdir()
        monkeypatch.setattr(taosmd_api, "_stores_cache", {})
        stores = asyncio.run(taosmd_api._ensure_stores(str(data_dir)))
        ...
        asyncio.run(taosmd_service.a2a_send("alice", "hello", thread="general", data_dir=str(data_dir)))

        env = os.environ.copy()
        env["TAOSMD_ONNX_PATH"] = "/nonexistent"
        result = subprocess.run(
            [sys.executable, "scripts/measure_channel_sender_stems.py", "--data-dir", str(data_dir)],
            capture_output=True,
            text=True,
            timeout=60,
            env=env,
    ...
    def test_data_dir_with_rows_succeeds(self, tmp_path, monkeypatch):
        ...
        asyncio.run(taosmd_service.a2a_send("alice", "hello", thread="general", data_dir=str(data_dir)))
        asyncio.run(taosmd_service.a2a_send("bob", "hi", thread="general", data_dir=str(data_dir)))
        asyncio.run(taosmd_service.a2a_send("@alice", "hey", thread="general", data_dir=str(data_dir)))
