...
def test_remove_then_cannot_readd_same_role(isolated_data_dir):
    """After removal, the principal is absent from list_members."""
    dd = str(isolated_data_dir)
    asyncio.run(service.a2a_create_thread("t1", ["alice", "bob"], "carol", data_dir=dd))
...
def test_http_a2a_add_member_non_owner(live_server):
    """POST /a2a/threads/{thread}/members by a non-owner returns 500 (PermissionError)."""
    _http_post(f"{live_server}/a2a/threads",
...
def test_http_a2a_remove_member_non_owner(live_server):
    """DELETE by a non-owner returns 500 (PermissionError)."""
    _http_post(f"{live_server}/a2a/threads",
