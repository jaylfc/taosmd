...
def test_create_thread_caller_not_in_participants(isolated_data_dir):
    ...
    carol = next(m for m in result["active_members"] if m["principal_id"] == "carol")
...
def test_add_member_by_owner(isolated_data_dir):
    ...
    members = asyncio.run(service.a2a_list_members("t1", data_dir=dd))
    dave = next(m for m in members if m["principal_id"] == "dave")
    assert dave["role"] == "member"
...
def test_archive_event_lands_on_add_and_remove(isolated_data_dir):
    ...
    Each call site is pinned separately by filtering on ``principal_id`` so
    that a no-op ``archive_membership_created`` at one site does not go red
    via events emitted by the other.
    """
    dd = str(isolated_data_dir)
    asyncio.run(service.a2a_create_thread("t1", ["alice", "bob"], "carol", data_dir=dd))
    ...
    try:
        ...
        created_principals: set[str] = set()
        removed_principals: set[str] = set()
        for e in events:
            data = json.loads(e.get("data_json", "{}"))
            action = data.get("admin_action")
            pid = data.get("principal_id")
            if action == "membership_created" and pid:
                created_principals.add(pid)
            if action == "membership_removed" and pid:
                removed_principals.add(pid)
...
# ---------------------------------------------------------------------------
# Removed-owner tests (pins removed_at IS NULL guards)
# ---------------------------------------------------------------------------

def test_removed_owner_cannot_pass_acl(tmp_path):
    """is_principal_owner must return False for a removed owner.

    Mutation: dropping ``removed_at IS NULL`` from the is_principal_owner
    query would let a removed owner keep passing the ownership ACL.
    """
    data_dir = tmp_path / "acl"
    data_dir.mkdir()
    store = MembershipStore(str(data_dir))
    try:
        asyncio.run(store.add_membership("t1", "alice", role="owner"))
        asyncio.run(store.add_membership("t1", "bob", role="member"))
        assert asyncio.run(store.is_principal_owner("t1", "alice")) is True
        asyncio.run(store.remove_membership("t1", "alice"))
        assert asyncio.run(store.is_principal_owner("t1", "alice")) is False
        assert asyncio.run(store.is_principal_owner("t1", "bob")) is False
    finally:
        asyncio.run(store.close())


def test_removed_owner_excluded_from_owners_list(tmp_path):
    """get_thread_owners must not include removed owners.

    Mutation: dropping ``removed_at IS NULL`` from the get_thread_owners
    query would let removed owners count toward the last-owner guard.
    """
    data_dir = tmp_path / "owners2"
    data_dir.mkdir()
    store = MembershipStore(str(data_dir))
    try:
        asyncio.run(store.add_membership("t1", "alice", role="owner"))
        asyncio.run(store.add_membership("t1", "dave", role="owner"))
        asyncio.run(store.remove_membership("t1", "alice"))
        owners = asyncio.run(store.get_thread_owners("t1"))
        assert len(owners) == 1
        assert owners[0].principal_id == "dave"
    finally:
        asyncio.run(store.close())


def test_remove_membership_idempotent_preserves_removed_at(tmp_path):
    """remove_membership must not overwrite an existing removed_at.

    Mutation: dropping ``removed_at IS NULL`` from the WHERE clause in
    remove_membership would let a re-removal overwrite the original
    removed_at timestamp, violating zero-loss.
    """
    ...
    try:
        asyncio.run(store.add_membership("t1", "alice", role="owner"))
        asyncio.run(store.remove_membership("t1", "alice", removed_at=100.0))
        row = store._conn.execute(
            "SELECT removed_at FROM a2a_membership WHERE thread='t1' AND principal_id='alice'"
        ).fetchone()
        assert row["removed_at"] == 100.0
        asyncio.run(store.remove_membership("t1", "alice", removed_at=200.0))
        row = store._conn.execute(
            "SELECT removed_at FROM a2a_membership WHERE thread='t1' AND principal_id='alice'"
        ).fetchone()
        assert row["removed_at"] == 100.0
...
# ---------------------------------------------------------------------------
# Routing guard tests (HTTP 404 for missing thread/principal)
# ---------------------------------------------------------------------------

def test_http_a2a_list_members_missing_thread(live_server):
    """GET /a2a/threads//members returns 404 (thread name required)."""
    status, body = _get(f"{live_server}/a2a/threads//members")
    assert status == 404
    assert "thread name required" in body.get("error", "")


def test_http_a2a_thread_messages_missing_thread(live_server):
    """GET /a2a/threads//messages returns 404 (thread name required)."""
    status, body = _get(f"{live_server}/a2a/threads//messages")
    assert status == 404
    assert "thread name required" in body.get("error", "")


def test_http_a2a_add_member_missing_thread(live_server):
    """POST /a2a/threads//members returns 404 (thread name required)."""
    status, body = _http_post(
        f"{live_server}/a2a/threads//members", {"principal_id": "dave", "agent": "carol"},
    )
    assert status == 404
    assert "thread name required" in body.get("error", "")


def test_http_a2a_remove_member_missing_thread_or_principal(live_server):
    """DELETE /a2a/threads//members/someone returns 404 (thread name required)."""
    status, body = _http_delete(
        f"{live_server}/a2a/threads//members/someone", {"agent": "carol"},
    )
    assert status == 404
    assert "thread name and principal required" in body.get("error", "")


# ---------------------------------------------------------------------------
# Percent-decoding test (%40 -> @)
# ---------------------------------------------------------------------------

def test_http_a2a_remove_member_percent_encoded_principal(live_server):
    """DELETE /a2a/threads/{t}/members/%40bob removes @bob (not not-found).

    Regression: without unquote(), the server sees the literal ``%40bob``
    in the path, fails to find the row, and returns 200 with
    ``removed: false, not_found: true`` -- a silent failure that reads as
    success at the status-code layer.
    """
    _http_post(f"{live_server}/a2a/threads",
               {"thread": "enc", "participants": ["@bob"], "agent": "carol"})
    status, body = _http_delete(
        f"{live_server}/a2a/threads/enc/members/%40bob", {"agent": "carol"},
    )
    assert status == 200, body
    assert body["removed"] is True
    assert body.get("not_found", False) is False

    status, body = _get(f"{live_server}/a2a/threads/enc/members")
    assert status == 200, body
    ids = [m["principal_id"] for m in body["members"]]
...
# ---------------------------------------------------------------------------
# Percent-decoding tests for thread name (the blocking defect of tsk-qzlijt)
# ---------------------------------------------------------------------------

def test_http_a2a_list_members_percent_encoded_thread(live_server):
    """GET /a2a/threads/{thread}/members with percent-encoded thread name.

    Regression: without unquote(), the server sees ``proj%20x`` literally,
    fails to match the stored thread ``proj x``, and returns 200 with an
    empty member list -- indistinguishable from an empty thread.
    """
    _http_post(f"{live_server}/a2a/threads",
               {"thread": "proj x", "participants": ["alice", "bob"], "agent": "carol"})
    status, body = _get(f"{live_server}/a2a/threads/proj%20x/members")
    assert status == 200, body
    ids = [m["principal_id"] for m in body["members"]]
    assert set(ids) == {"alice", "bob", "carol"}


def test_http_a2a_add_member_percent_encoded_thread(live_server):
    """POST /a2a/threads/{thread}/members with percent-encoded thread name."""
    _http_post(f"{live_server}/a2a/threads",
               {"thread": "proj y", "participants": ["alice"], "agent": "carol"})
    status, body = _http_post(
        f"{live_server}/a2a/threads/proj%20y/members",
        {"principal_id": "dave", "agent": "carol"},
    )
    assert status == 200, body
    assert body["added"] is True
    status, body = _get(f"{live_server}/a2a/threads/proj%20y/members")
    assert status == 200, body
    ids = [m["principal_id"] for m in body["members"]]
    assert "dave" in ids


def test_http_a2a_remove_member_percent_encoded_thread(live_server):
    """DELETE /a2a/threads/{thread}/members/{principal} with percent-encoded thread."""
    _http_post(f"{live_server}/a2a/threads",
               {"thread": "proj z", "participants": ["alice", "bob"], "agent": "carol"})
    status, body = _http_delete(
        f"{live_server}/a2a/threads/proj%20z/members/alice", {"agent": "carol"},
    )
    assert status == 200, body
    assert body["removed"] is True
    status, body = _get(f"{live_server}/a2a/threads/proj%20z/members")
    assert status == 200, body
    ids = [m["principal_id"] for m in body["members"]]
    assert "alice" not in ids
    assert "bob" in ids
...
# ---------------------------------------------------------------------------
# End-to-end probe: RemoteClient against a live server, space in thread name
# ---------------------------------------------------------------------------

def test_remote_membership_percent_encoded_thread(live_server):
    """End-to-end: RemoteClient round-trips membership ops on a thread whose
    name contains a space, exercising client encoding + server decoding on all
    three routes plus the list path that returns empty without the fix.
    """
    from taosmd.remote import RemoteClient
    rc = RemoteClient(live_server)
    thread = "proj x"

    # Create the thread via RemoteClient (thread name goes in JSON body).
    result = asyncio.run(rc.a2a_create_thread(thread, ["alice", "bob"], "carol"))
    assert result["thread"] == thread
    assert result["created"] is True

    # List members: without unquote(), server sees "proj%20x" and returns [].
    members = asyncio.run(rc.a2a_list_members(thread=thread))
    ids = {m["principal_id"] for m in members}
    assert ids == {"alice", "bob", "carol"}, f"expected 3 members, got {ids}"

    # Add a member (-- without quote(), raw f-string raises InvalidURL).
    result = asyncio.run(rc.a2a_add_member(thread, "dave", "carol"))
    assert result["added"] is True
    members = asyncio.run(rc.a2a_list_members(thread=thread))
    ids = {m["principal_id"] for m in members}
    assert "dave" in ids

    # Remove a member (-- without quote(), raw f-string raises InvalidURL).
    result = asyncio.run(rc.a2a_remove_member(thread, "alice", "carol"))
    assert result["removed"] is True
    members = asyncio.run(rc.a2a_list_members(thread=thread))
    ids = {m["principal_id"] for m in members}
    assert "alice" not in ids
