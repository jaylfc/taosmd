...
def test_legacy_modern_archive_index_is_stamped_without_reapplying(tmp_path):
    ...
    conn.execute(
        ...
        " file_path, line_number, data_json, source, source_id) "
        "VALUES (1.0, 'turn', 'a', 'app', 'proj', 's', 'f.jsonl', 1, '{}', 'src', 'sid-1')"
    )
    conn.commit()
...
# ----------------------------------------------------------------------
# source_uid migration: fresh install, upgrade
# ----------------------------------------------------------------------


@pytest.mark.asyncio
async def test_fresh_archive_index_has_source_columns_and_index(tmp_path):
    """Fresh install: source/source_id columns and idx_archive_source_uid present.

    Includes the positive control: 'project' is also present, so a failure
    reads as 'this migration did not run' rather than 'migrations are broken'.
    """
    from taosmd.archive import ArchiveStore
    ...
    try:
        ...
        assert "project" in cols, "positive control: migration 2 must have run"
        assert "source" in cols, "source column missing after fresh init"
        assert "source_id" in cols, "source_id column missing after fresh init"
        idx = {r[0] for r in conn.execute(
            "SELECT name FROM sqlite_master WHERE type='index'")}
        assert "idx_archive_source_uid" in idx
        assert _user_version(conn) == migrations.latest_version("archive_index")
    finally:
        conn.close()


def test_upgrade_adds_source_columns_and_index(tmp_path):
    """Upgrade: legacy DB gains source/source_id and the partial unique index."""
    path = tmp_path / "archive-index.db"
    conn = _db.connect(path)
    conn.executescript(
        """
    ...
    conn.commit()
    assert "source" not in _columns(conn, "archive_index")
    assert "source_id" not in _columns(conn, "archive_index")
