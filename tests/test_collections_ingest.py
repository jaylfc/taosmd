...
def test_parse_front_matter_version_non_int_is_skipped(tmp_path):
    """DEFECT 5: version must be int or absent, never str."""
    p = _fm(tmp_path, '---\nversion: 2.5\n---\nbody\n')
    fm = _parse_front_matter(str(p))
...
def test_parse_front_matter_review_by_invalid_date_is_skipped(tmp_path):
    """DEFECT 5: review_by must be a valid ISO date."""
...
def test_parse_front_matter_thematic_break_not_captured(tmp_path):
    """DEFECT 6: a document that opens with a thematic break (---) must not
    have its prose scanned as front matter, even if a later --- appears."""
    text = (
        "---\n\n"
        "# Title\n\n"
    ...
    p = _fm(tmp_path, text)
    fm = _parse_front_matter(str(p))
    # The closing --- is ~5 lines down; with the line budget, this is treated
    # as a thematic break, not front matter. version in prose is NOT captured.
    assert "version" not in fm
...
def test_ingest_folder_front_matter_reaches_search(data_dir, source_dir):
    """BLOCKER 1 + DEFECT 4: end-to-end front-matter survives ingest_folder ->
    search -> _format_hit with the real data shape."""
    _patch_embedder(data_dir)
...
def test_ingest_folder_markdown_extension_front_matter(data_dir, source_dir):
    """DEFECT 6: .markdown files also get front-matter parsing."""
    _patch_embedder(data_dir)
