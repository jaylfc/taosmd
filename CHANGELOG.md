...
Databases already in the field report `user_version = 0` while carrying a fully modern schema, because nothing ever stamped them, and replaying every step against a live store is the obvious way to break one. So each migration carries a `detect` probe, a cheap `PRAGMA table_info`-style read answering "is this change already present?". On an unstamped database the runner walks the list and stamps to the longest leading run of steps that report themselves already present, executing none of them, then applies only the remainder. One mechanism covers every case: an empty file detects nothing and is built from step 1, a modern legacy store detects everything and is stamped straight to the latest version with zero writes to its schema, and a mid-era store is stamped to its prefix with the tail applied. Tests seed genuine legacy databases (pre-`project` archive index, pre-taxonomy session catalog, pre-`valid_to` vector store, an unstamped Phase 1 `collections.db`) and assert both the resulting version and, through a spy on every step, that nothing was re-run; a failing migration is proven to roll back its schema change and its version stamp together, leaving neither half applied.

The three hand-rolled guards are subsumed as ordered steps with their exact prior behaviour on both fresh and existing databases: the archive `project` back-fill, the session-catalog taxonomy columns plus the `agent_name` column and the taxonomy index (which must still be created after the columns it indexes exist), and `VectorMemory._migrate`'s `valid_to` column plus its deferred `idx_vm_valid` index. `collections.db` gains its Phase 1 schema as version 1. Adding a whole new table can still live in a store's `SCHEMA` constant, since `IF NOT EXISTS` genuinely reaches existing databases; changing an existing table must now be a migration, and `docs/migrations.md` states that contract along with the idempotency and transaction rules a step must obey.

New CLI: `taosmd migrate` applies pending migrations to a data directory, `taosmd migrate --status` prints each database's version against the latest known one, and `taosmd migrate --check` is a dry-run deploy pre-flight that writes nothing and exits non-zero when any database is behind, so a rollout can refuse to start against a store it would silently need to upgrade. `--json` gives all of it machine-readably.

Benchmark harness: the EventQA runner (`benchmarks/eventqa_runner.py`) routes its vector stage through `taosmd.retrieval.retrieve()` instead of calling `vmem.search()` directly, which is what makes the `graph_expansion` control reachable from that benchmark at all. Adds `--graph-expansion`, `--retrieval-path` and `--report-retrieval-delta`, plus a `retrieve_supports_graph_expansion()` signature probe that refuses a non-zero `--graph-expansion` on a checkout whose `retrieve()` lacks the parameter, rather than silently running two identical arms. The parameterisation reconstructs the old stage deliberately (`candidate_top_k` pinned to the previous fetch width, `limit` narrowed only when a reranker is live), and a delta check confirmed byte-identical context against the legacy path on 25 of 25 real questions. One residual difference is disclosed rather than hidden: `retrieve()` applies a near-duplicate filter the old path lacked, which is why `--report-retrieval-delta` exists. Note for anyone bisecting: this landed in merge commit `ea882a2`, whose message describes only the `docs/collections.md` change it was bundled with.

Docs: `docs/collections.md` documents the response envelope for every collection endpoint (`{"collection": {...}}`, `{"collections": [...]}`), the fields of the collection object, and why a status-code probe for collections support is unsafe (unknown paths fall through to the dashboard SPA with `200 text/html`, so the check passes on builds with no collections code). The envelope had only ever been recorded in the `http_server.py` docstring table, so an integrator reading the user-facing page parsed responses unwrapped, read an empty status, and stalled a poll loop on collections that had indexed correctly.
...
Graph-expansion fact readback (bi-temporal Phase 1, default-off / opt-in). Wires the previously orphaned `graph_expansion.expand_from_results` into `retrieve()`: when the new `graph_expansion` control is on and a `"kg"` source is present, the entities in the served hits are looked up in the already-populated bi-temporal knowledge graph and their currently-valid facts are appended as one derived-facts block. This reads the fact store back into the answer context, which was write-only at answer time. Provenance stays honest: the block is labelled derived (`source == "kg_expansion"`, `derived == True`, confidence carried from the strongest triple), the underlying turns remain the primary evidence, the claims gate is untouched, and no new storage, schema, or extraction change is introduced. Phase 1 is valid-time only (`query_entity` defaults to `as_of=now`); no `as-of` query surface is exposed yet (that is Phase 2). The `graph_expansion` control is an int token budget for the block (`0` is off, the default); wired through the runtime controls into `search()` like `adjacent_turns`. The stage is fail-open: a missing, empty, or erroring KG is a silent no-op, and with the control off the retrieval path is byte-for-byte unchanged. EventQA validation (E-025, pre-registered) is pending before any default flip.
