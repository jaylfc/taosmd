...
`factual-recall` generator profile updated to granite4.1:8b at the 12 and 8 GB tiers. On the corrected LongMemEval-S full-500 shipped-combo config granite4.1:8b scores 54.8 (strict Qwen judge) / 60.4 (cross-family llama judge), a statistical tie with the prior 12 GB pick gemma4:12b (53.8 / 61.4) and with MemOS-lossless (54.0 / 61.2), at 4.9 GB q4 under Apache-2.0; the VRAM headroom and license break the tie at 12 GB, and at 8 GB it decisively beats the prior llama3.1:8b pick (49.2 / 54.4). The 4 GB pick stays llama3.1:8b (granite does not fit a 4 GB card). The `balanced` default is unchanged: granite4.1:8b failed the pre-registered LoCoMo kill criterion (0.475 vs qwen3.5:9b 0.515, external judge), so it ships as a single-fact-QA profile pick only. Evidence: research report F-016/E-024, `benchmarks/results/20260705-screening/`, `benchmarks/results/20260706-locomo-granite/`.
